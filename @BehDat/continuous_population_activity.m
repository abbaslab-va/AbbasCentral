function [spikeMat, sortedIdx] = continuous_population_activity(obj, varargin)
...
numSamples = floor(abs(diff(presets.edges) / obj.info.baud * 1000 / presets.binWidth));
spikeMat = zeros(numNeurons, numSamples);

if isscalar(obj.find_event('preset', presets))
    spikeMat = obj.bin_all_neurons('preset', presets);
else
    for i = 1:numNeurons
