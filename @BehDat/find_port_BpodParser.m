function portInfo = find_port_BpodParser(obj, varargin)
...
addParameter(p, 'proximalEdges', [-1 0], validVectorSize)
parse(p, varargin{:});
...
proxStateTimes(trialHasState) = proxStateEdges;
[proxStateTimes{~trialHasState}] = deal({[-2, -1]});
eventProximal = cellfun(@(x, y) cellfun(@(z) discretize(y, z), x, 'uni', 0), proxStateTimes, allEventTimes, 'uni', 0);
...
allEventTimes = obj.bpod_to_blackrock(allEventTimes, presets);
...
portTimes = struct('previous', prevEventTimes, 'current', allEventTimes, 'next', nextEventTimes);
...
portInfo = struct('times', portTimes, 'reward', portRewards, 'identity', portID, 'included', eventIncluded, 'proximal', eventProximal);