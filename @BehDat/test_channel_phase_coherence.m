...
% temperature and harmonic structure analyses
    ...
    signalClusters = cluster(linkageData, 'maxclust', numClusters);
    maxCrossings = 0;
    maxClust = 0;
    ...
    regionIdx = signalClusters == maxClust;
    goodChannels = obj.info.channels.(presets.region)(regionIdx');
    goodSignal = cellfun(@(x) mean(x(:, regionIdx), 2), filteredSignal, 'uni', 0);
    ...
    % figure
    % plot(goodSignal{1}, 'LineStyle', '--', 'LineWidth', 2, 'Color', 'k')
    % hold on
    % plot(filteredSignal{1}(:, regionIdx))
